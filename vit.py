...
class PatchEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        # Use Conv2d instead of patchify
...
class Embeddings(nn.Module):
    def __init__(self, config):
        ...
        # Random number [CLS] token pre-embedded as "hidden_size"
        # The [CLS] token is added to the beginning of the input sequence
        self.cls_token = nn.Parameter(torch.randn(1, 1, config["hidden_size"]))
        
        # Create position embeddings initialize to be zeros or small numbers (borrowed from timm)
...
class ViTForClassfication(nn.Module):
    ...
    def forward(self, x, output_attentions=False):
        ...
        #fc1 = self.linear1(encoder_output[:, 0, :])
